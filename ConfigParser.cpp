...
void ConfigParser::_processConfig(std::string argv) {
    if (argv.substr((argv.size() - strlen(".conf")), strlen(".conf")) != ".conf")
		...
		if(!this->isEnoughInfo())
            throw std::runtime_error("Not enough info provided in configuration file");
    }
}

bool ConfigParser::isEnoughInfo() {
    for (std::size_t i=0; i < this->_serversArray.size(); i++){
        if (this->_serversArray[i]._host.empty() && this->_serversArray[i]._serverName.empty())
            return false;
        if (this->_serversArray[i]._locations.empty())
            return false;
        for (std::size_t j=0; j<this->_serversArray[i]._locations.size(); j++){
            if (this->_serversArray[i]._locations[j]._root.empty() && this->_serversArray[i]._locations[j]._index.empty())
                return false;
        }
    }
    return true;
...
//
//// str27
void ConfigParser::processBlocks(std::string block, int server_index, bool is_loc){
	std::size_t i=0;
	int dir_type;
    std::string type;
...
//            dir_type = this->directiveTypes(block.substr(i, block.substr(i).find_first_of(" \t\n")));
			...
			if (dir_type==LOCATION && is_loc == true)
				...
				i += this->processInstructions(dir_type, block.substr(i),
											  server_index, is_loc);
		}
		i++;
	}
...
int ConfigParser::processInstructions(int type, std::string dir, int ind, bool is_loc){
	std::size_t args;
	std::size_t dir_len;
...
std::size_t ConfigParser::processLocation(std::string dir, int ind){
	...
	this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size() - 1]._url = args[1];
	this->copyValues(this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size() - 1], ind);
...
std::vector<std::string> ConfigParser::extractArgs(std::string dir, char delim){
	std::vector<std::string> args;
	int i=0;
	int size_args=0;
...
void ConfigParser::processListen(std::string dir, int ind, int arg_num, bool is_loc){
	std::vector<std::string> args;
	int 	port;
	std::string port_str;
...
void ConfigParser::processRoot(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string> arg;

    if (arg_num != 2)
        throw std::runtime_error("One argument for root must be provided");
    arg = this->extractArgs(dir, ';');
    if (is_loc && !this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size() - 1]._root.empty())
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size() - 1]._root.clear();
    else if (!this->_serversArray[ind]._root.empty())
        this->_serversArray[ind]._root.clear();
    if (is_loc)
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._root = arg[1];
    else
        this->_serversArray[ind]._root = arg[1];
}

void ConfigParser::processErrorPage(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string>    args;
    std::vector<std::string>    err;

    if (arg_num < 3)
        throw std::runtime_error("At least two arguments for error_page must be provided");
    args = this->extractArgs(dir, ';');
    for (std::size_t i=1; i<args.size(); i++)
        err.push_back(args[i]);
    if (is_loc)
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._errorPages.push_back(err);
    else
        this->_serversArray[ind]._errorPages.push_back(err);
}

void ConfigParser::processBodySize(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string>    args;
    int     mult=1;
    int     body_size_int;

    if (arg_num != 2)
        throw std::runtime_error("Two arguments for client_body_size must be provided");
    args = this->extractArgs(dir, ';');
    ...
    if (is_loc)
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._bodySize = body_size_int;
    this->_serversArray[ind]._bodySize = body_size_int;
}

void ConfigParser::processAutoindex(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string> args;
    bool flag= false;
    ...
    if (!args[1].compare("on"))
        flag= true;
    if (is_loc)
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size() - 1]._autoindex = flag;
    else
        this->_serversArray[ind]._autoindex = flag;
}

void ConfigParser::processIndex(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string> args;
    ...
    if (is_loc && !this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._index.empty())
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._index.clear();
    else if (this->_serversArray[ind]._index.empty())
        this->_serversArray[ind]._index.clear();
    if (is_loc)
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._index = values;
    else
        this->_serversArray[ind]._index = values;
}

void ConfigParser::processAllow(std::string dir, int ind, int arg_num, bool is_loc) {
    std::vector<std::string> args;

    if (arg_num < 2)
        throw std::runtime_error("Wrong number of aruments in allow_methods");
    args = this->extractArgs(dir, ';');
    if (is_loc && !this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._allowedMethods.empty())
        this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._allowedMethods.clear();
    ...
    for (std::size_t i=1; i<args.size(); i++){
        if (args[i].compare("GET") && args[i].compare("POST")) // TODO DELETE???
            throw std::runtime_error("Wrong type of expression provided for allowed_methods");
        if (is_loc)
            this->_serversArray[ind]._locations[this->_serversArray[ind]._locations.size()-1]._allowedMethods.push_back(args[i]);
        else
            this->_serversArray[ind]._allowedMethods.push_back(args[i]);
    }
}

void ConfigParser::processServerName(std::string dir, int ind, int arg_num, bool is_loc){
	std::vector<std::string> args;

	if (arg_num < 2)
		throw std::runtime_error("Wrong amount of arguments in server_name directive");
	if (is_loc)
